...
class BlueWrapper(BaseWrapper):
    ...
    def observation_change(self, observation, baseline=False):
        ...
        self._process_last_action(success)
    ...
    def _process_last_action(self, success):
        ...
        if action is not None:
            ...
            if name == "Restore":
                ...
                if compromised == "User":
                    self.blue_info[hostname][-1] = "No"
            # new line to change the status of a "User" machine to "Unknown" if it was not removed
            for host in self.blue_info:
                compromised = self.blue_info[host][-1]
                if compromised == "User":
                    self.blue_info[host][-1] = "Unknown"

    # old version of _process_last_action
    def _old_process_last_action(self):
        action = self.get_last_action(agent="Blue")
        if action is not None:
            name = action.__class__.__name__
            hostname = (
    ...
    # These Vectors have been changed for FSP, so that one boolean now represents only 1 value
    def _create_vector(self, success):
        table = self._create_blue_table(success)._rows

        # MULTIDISCRETE ATTEMPT
        # proto_vector = np.array([0] * 24)
        ...
        #     compromised = row[4]
        #     if compromised == "No":
        #         proto_vector[2 * r + 1] = 0
        #     elif compromised == "User":
        #         proto_vector[2 * r + 1] = 1
        #     elif compromised == "Unknown":
        #         proto_vector[2 * r + 1] = 2
        #     elif compromised == "Privileged":
        #         proto_vector[2 * r + 1] = 3
        #     else:
        #         raise ValueError("Table had invalid Access Level")
    ...
    # old function using the original features
    # def _old_create_vector(self, success):
    #     table = self._create_blue_table(success)._rows
