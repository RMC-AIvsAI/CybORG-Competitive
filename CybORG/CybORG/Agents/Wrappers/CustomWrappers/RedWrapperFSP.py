...
class RedWrapper(BaseWrapper):
    def __init__(self, host_list, env=None, agent=None, output_mode="table"):
    ...
    def _create_vector(self):
        ...
        # success flag followed by individual host vectors
        success_value = int(self.success.value) if self.success.value < 2 else 0
        proto_vector = [success_value]

        # Compute required length of vector based on number of hosts
        num_hosts = len(self.host_list)
        proto_vector.extend(num_hosts * 4 * [0])

        # vector of flags for each host -> 0 0 0 0 0
        for row in table:
            # 1st flag represents if that host is discovered
            if row[2] in self.host_list:
                index = self.host_list.index(row[2])
                proto_vector[4 * index + 1] = 1

                # 2nd flag represents if that host has been scanned
                proto_vector[4 * index + 2] = int(row[3])

                # 3rd flag represents User access
                # 4th flag represents Privileged access
                # Access
                access = row[4]
                if access == "User":
                    proto_vector[4 * index + 3] = 1
                elif access == "Privileged":
                    proto_vector[4 * index + 4] = 1
                else:
                    raise ValueError("Table had invalid Access Level")

        return np.array(proto_vector)

    def _old_create_vector(self, num_hosts=13):
        table = self._create_red_table()._rows
