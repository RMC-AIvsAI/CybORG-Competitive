...
class CybORG (CybORGLogger):
    ...
    def old_step(self, agent: str = None, action=None, skip_valid_action_check: bool = False) -> Results:   #### old step function from original CybORG 
        ...
        """
        return self.environment_controller.old_step(agent, action, skip_valid_action_check)

    def step(self, red_step=None, blue_step=None, skip_valid_action_check: bool = False) -> Results:   #### new step function for competitive CybORG
       
        return self.environment_controller.step(red_step, blue_step, skip_valid_action_check)
