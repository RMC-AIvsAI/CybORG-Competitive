...
class EnvironmentController:
    ...
    def reset(self, agent: str = None) -> Results:
        ...
        return Results(observation=self.init_state, blue_observation=self.observation['Blue'].data, red_observation=self.observation['Red'].data)

    def old_reset(self, agent: str = None) -> Results:
        """Resets the environment and get initial agent observation and actions.
    ...
    def step(self, red_step: Action = None, blue_step: Action = None, skip_valid_action_check: bool = False) -> Results:     #### Step for competetive CybORG
        ...
        for agent_name, agent_object in self.agent_interfaces.items():
            ...
            if agent_name == 'Red':
                if red_step is None:
                    ...
                    agent_action = red_step

            if agent_name == 'Blue':
                if blue_step is None:
                    agent_action = agent_object.get_action(self.observation[agent_name])
                else:
                    agent_action = blue_step
            ...
            if not self.test_valid_action(agent_action, agent_object) and not skip_valid_action_check:
                ...
                #print(f'Invalid action selected by {agent_name}')
        ...
        # ** NEW ** for the zero-sum game, set the red reward to be the opposite of blue
        self.reward['Red'] = -self.reward['Blue']
        ...
        #if agent is None:
        result = Results(observation=true_observation, blue_observation=self.observation['Blue'].data, red_observation=self.observation['Red'].data, 
                            done=self.done, reward=round(self.reward['Blue'], 1))
        # else:
    ...
    def old_step(self, agent: str = None, action: Action = None, skip_valid_action_check: bool = False) -> Results:     #### Step from original CybORG
